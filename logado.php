...
if(!isset($_SESSION['valido'])){
	...
	if(!$_SESSION['valido'] || ($u != $_SESSION['nuser'])){
		...
		if(mysql_num_rows($result) != 0){
			...
			$con;
			// Comprobacion de si tiene mas de 10 pj o estan pendientes de recuperación.
			if($ser == 'aura'){
				$con = mysql_connect("$hostchar_subaura","$user","$pass") or die("Error en la conexion de la bd");
			}
			else{
				$con = mysql_connect("$hostchar_subuxia","$user","$pass") or die("Error en la conexion de la bd");
			}
			mysql_select_db("$dbc",$con);
			$sqlcount = "SELECT count(guid) as cuenta FROM characters WHERE account = ".$fila['id'].";";
			$restcount = mysql_query("$sqlcount",$con) or die("Error en la sentencia");
			echo mysql_fetch_assoc($restcount)['cuenta']."<br>";
			if(mysql_fetch_assoc($restcount)['cuenta'] >= 10){
				$acto = false;
			}
		}
		else{
...
if($_SESSION['valido'] && $acto){
	header('Location: index.php');
}
else{
	header('Location: subcant.php');
}
?>